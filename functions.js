...
function subtraction(pixelA, pixelB) {
  return {
    r: pixelA.r - pixelB.r,
    g: pixelA.g - pixelB.g,
    b: pixelA.b - pixelB.b,
...
function pixelToCipherBits(pixel) {
  return Object.values(pixel)
    .map((byte) => byte.toString(2))
    .join('');
}

function codeXorBits(cipherBits, keyBits) {
  const cipherBitsArray = cipherBits.split('');
  const keyBitsArray = keyBits.split('');
  return cipherBitsArray
    .map((bit, i) => {
      const keyBit = keyBitsArray[i];
      return bit ^ keyBit;
    })
    .join('');
}
